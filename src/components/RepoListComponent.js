...
const RepoListComponent = () => {
  ...
  const fetchRepo = async () => {
    try {
      const response = await axios.get(
        `https://api.github.com/users/nikolasdevs/repos?page=${page}&per_page=6`
      );
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmation) {
      try {
        axios.delete(`http://localhost:3004/${id}`).then((res) => {
  ...
  return (
    <div className="heights bg-gray-900 w-full pt-2 relative">
      ...
      <div className="flex flex-col mt-16">
          ...
          {filtered.map((repoItem) => (
            ...
            >
              ...
              <div className=" flex justify-between w-full mt-8 ">
                ...
                <p
                  onClick={() => handleDelete(repoItem.name)}
        ...
        <div className="flex items-center justify-center gap-16 my-12 mx-auto w-full flex-col md:flex-row">
